...
public class MainActivity extends AppCompatActivity {
    ...
    private void updateSpectrumData() {
        if(playing) {
            ...
            filterSeries.setBackgroundColor(bgColor);
            filterSeries.setColor(bgColor);
            //correctionSeries.setColor(Color.WHITE);
    ...
    private void updateFilterData() {
        if(playing) {
            ...
            for (int i = 0; i < n_freqs; i++) {
                DataPoint v = new DataPoint(log2(filterFrequencies[i]), spectrum[i]);
                //Log.i("fr","" + spectrumFrequencies[i] + " "+spectrum[i]);
                values[i] = v;
                //corrVal[i] = new DataPoint(log2(filterFrequencies[i]), corr[i]);
            }
